...
/**
 *    @title TimeLock ERC20 Token
 *    @notice A token whose some amount of tokens can be locked for some time
 *    @dev Uses OpenZeppelin's ERC20, Ownable, IERC20, TokenTimelock and Pausable for standard functionalities
 *    @notice TokenTimelock.sol is deprecated from OpenZeppelin's smart contract library
 */
contract TimeLockToken is ERC20, Pausable, Ownable {
    ...
    /**
     * @dev Lock a specific amount of tokens for a beneficiary
     * @param beneficiary The address to receive the tokens after the release time
     * @param amount The amount of tokens to lock
     */
    function timeLock(address beneficiary, uint256 amount) external onlyOwner {
        require(
            amount <= balanceOf(address(this)),
            "Insufficient balance in contract"
        );

        uint256 releaseTime = block.timestamp + 70; // Set to 70 seconds for the test
